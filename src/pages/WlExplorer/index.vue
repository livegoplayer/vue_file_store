...
import wlTreeSelect from '@/components/wl-tree'
import { arrayToTree, splicParentsUntil, download } from '@/util' // 导入组装树函数、拼接路径函数
const guid = '00000000-0000-0000-0000-000000000000'
export default {
  name: 'wlExplorer',
  components: { submitBtn, fileView, fadeIn, uploadItem, wlTreeSelect },
  data () {
    return {
      load: {
        del: false, // 删除
  ...
  methods: {
    ...
    fileTypeIcon (row) {
      ...
      // 其他根据后缀类型
      const _suffix = row[this.selfProps.name].substring(row[this.selfProps.name].lastIndexOf('.') + 1)
      if (!_suffix) {
  ...
  computed: {
    ...
    selfProps () {
      return {
        ...
        pathName: 'path_name', // String 路径数据 显示名称字段
        pathId: 'id', // String 路径数据 id字段
        pathPid: 'parent_id', // String 路径数据 pid字段
